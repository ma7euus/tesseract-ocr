...
void TessBaseAPI::SetIDUser(const char* _id_user) {
  if (id_user_ == NULL)
    id_user_ = new STRING(_id_user);
  else
    *id_user_ = _id_user;
}

void TessBaseAPI::SetIDImage(const char* _id_img) {
  if (id_img_ == NULL)
    id_img_ = new STRING(_id_img);
  else
    *id_img_ = _id_img;
}

const char* TessBaseAPI::GetIDImage(){
    return id_img_->string();
}

const char* TessBaseAPI::GetIDUser(){
    return id_user_->string();
}
...
//    Boxa* wordsBoxa = GetWords(NULL);
//    
//    for(int i = 0; i < wordsBoxa->n; i++){
//       Pix* pix = pixClipRectangle(tesseract_->pix_binary(), wordsBoxa->box[i], NULL);
//       char buf[256];
//       snprintf(buf, sizeof(buf), "/tmp/teste/word.%02d.jpg", i);
//       pixWrite(buf, pix, IFF_JFIF_JPEG);
//       pixDestroy(&pix);
//       boxDestroy(&wordsBoxa->box[i]);
//    }
...
char* TessBaseAPI::GetUTF8Text() {
  ...
  int word_count = 1;
  do {
    if (it->Empty(RIL_BLOCK)) continue;
    char *para_text = it->GetUTF8Text_APP(RIL_BLOCK, GetIDUser(), GetIDImage(), &word_count);
    text += para_text;
    delete []para_text;
