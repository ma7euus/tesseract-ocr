...
bool Tesseract::recog_all_words(PAGE_RES* page_res,
  ...
  if (dopasses==0 || dopasses==1) {
    ...
    while (page_res_it.word() != NULL) {
      ...
      //tprintf("Pass%d\n", i++);
      //page_res_it.word()->word->bounding_box()->plot();
...
//          const int pix_height = pixGetHeight(word->tesseract->pix_binary());
//          const int pix_width = pixGetWidth(word->tesseract->pix_binary());
...
//          Pix* pix = pixClipRectangle(BestPix(), bbox, NULL);
//          char buf[256];
    /*
...
//    tprintf("%s : %d\n",word->best_choice->unichar_string().string(), pix->data);
//    snprintf(buf, sizeof(buf), "/tmp/teste/%d.jpg", pix->data);
